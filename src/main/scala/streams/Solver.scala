...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for {
      nb <- b.legalNeighbors
    } yield (nb._1, nb._2 :: history)).to(LazyList)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    for {
      pathSets <- pathsFromStart
      if pathSets._1 == Block(goal, goal)
    } yield pathSets
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.take(1).flatMap(x => x._2).reverse.toList
}
